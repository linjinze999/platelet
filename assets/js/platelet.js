...
function showMessage(text, timeout, flag) {
  if (
    flag ||
    sessionStorage.getItem("platelet-text") === "" ||
    sessionStorage.getItem("platelet-text") === null
  ) {
...
function formatSeconds(value) {
  ...
  var result = "";
  if (minutes > 0) result = "" + parseInt(minutes) + "分";
  if (hours > 0 && hours <= 24) result = "" + parseInt(hours) + "小时" + result;
  if (days > 0) result = "" + parseInt(days) + "天" + result;
  return result;
}
  ...
  if (is_play) {
    $(".music").html(
      '<audio controls="controls" id="audio_player" style="display:none;"> <source src="' +
        ...
        file +
        '" loop="false" width="0px" height="0px" /></embed>'
    );
  } else {
    $(".music").html("");
  }
